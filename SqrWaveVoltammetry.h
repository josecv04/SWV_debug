...
// adding here -jose 
#ifndef SQRWAVEVOLTAMMETRY_H   // use a unique, conflict-free guard
#define SQRWAVEVOLTAMMETRY_H

#ifdef __cplusplus
extern "C" {
#endif

#include "ad5940.h"    // <-- REQUIRED so AD5940Err, BoolFlag, SEQInfo_Type, etc. exist
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include <math.h>
 ...
 * --------------------------
 * These functions allow the host (ESP32) to label each FIFO sample with the
 * SWV phase as tracked inside the SWV engine, rather than inferring phase from
 * a host-side sample index parity.
 *
 * Phase convention used here:
 *   - 'F' (forward)  = square-wave HIGH level
 *   - 'R' (reverse)  = square-wave LOW  level
 */
