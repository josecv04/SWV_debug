 ...
 * ------------------------
 * We keep a small, host-facing state machine that advances once per *returned*
 * FIFO sample. This lets the ESP32 attach a phase bit that is derived from the
 * SWV engine's phase progression rather than from host-side parsing/parity.
 *
 * IMPORTANT: This assumes the SWV engine produces one FIFO sample per square-
 * wave half-cycle and that phases alternate HIGH/LOW for successive samples.
 * If you later change the sequencer to average multiple samples per half-cycle
 * or otherwise alter the FIFO emission pattern, update this logic accordingly.
 */
static uint32_t g_stream_sample_idx = 0;
static BoolFlag g_stream_hi_level = bTRUE; /* first returned sample corresponds to HIGH level */

void AppSWV_StreamReset(void)
{
  g_stream_sample_idx = 0;
  /*
   * The default SWV config starts with AppSWVCfg.bSqrWaveHiLevel = bFALSE.
   * RampDacRegUpdate() toggles the flag and produces the first DAC code such
   * that the first measurement corresponds to the HIGH level. Therefore we
   * start the streaming phase as HIGH.
   */
  g_stream_hi_level = bTRUE;
}

char AppSWV_StreamNextPhaseChar(void)
{
  char ph = g_stream_hi_level ? 'F' : 'R';
  g_stream_hi_level = g_stream_hi_level ? bFALSE : bTRUE;
  g_stream_sample_idx++;
  return ph;
}

uint32_t AppSWV_StreamNextStepIndex(void)
{
  /* Two half-cycles (F then R) per staircase step */
  ...
  case APPCTRL_START:
    {
      ...
      */
      ...
      /* Reset host-facing stream metadata at the start of a run */
      AppSWV_StreamReset();
...
AD5940Err AppSWVISR(void *pBuff, uint32_t *pCount)
{
  ...
  *pCount = 0;
  ...
  if(IntFlag & AFEINTSRC_DATAFIFOTHRESH)
  {
    ...
    {
      /* Buffer limited; existing behavior. */
    }
